...
<table class="table-component table-auto w-full mt-8">
  ...
  <tbody>
    {#each rows as row, idx}
      <tr class="{row.status.toUpperCase().indexOf('ERROR') !== -1 || row.status === 'failure' ? 'highlight' : ''}">
        <td data-label="script" class="px-4 py-2">{row.table_name}</td>
        <td data-label="date" class="px-4 py-2">{new Date(row.update_time).toLocaleString()}</td>
